...
// Load shared helpers before any consumer touches them.
...
/**
 * Ensure WooCommerce product grids align with the global grid helpers.
 *
 * @param string $html Opening markup generated by WooCommerce.
 * @return string
 */
function beyond_gotham_enhance_product_loop_markup( $html ) {
    if ( false === strpos( $html, 'products' ) ) {
        return $html;
    }

    if ( false !== strpos( $html, 'bg-grid' ) ) {
        return $html;
    }

    return str_replace( 'products', 'products bg-grid bg-grid--shop', $html );
}
add_filter( 'woocommerce_product_loop_start', 'beyond_gotham_enhance_product_loop_markup' );
